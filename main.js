...
function templateListButton(filelist){
  var buttonList = '<ul class="row justify-content-center w-100 mx-0 list-inline">';
  for(var i=0; i<filelist.length; i++){
      buttonList += `<li class="col-3 col-md-2"><a href="/?id=${filelist[i]}"><button type="button" class="Portfolio_TextButton rounded w-100">${filelist[i]}</button></a></li>`;
  }
  buttonList += '</ul>';
  return buttonList;
}
function templateListProject(filelist){
  var list = ``;
  for(var i=0; i<filelist.length; i++){
...
function templateHTML(buttonList, title, body){
  return `
  <!doctype html>
  <html>
  <head>
  ...
  <main>
    <section class="py-5 text-center container">
      <div class="profile-img rounded-circle p-2">
        <img class="rounded-circle w-100 h-100"src="http://localhost:8080/img/profile.jpg" alt="profile image">
      </div>
    ...
    if(pathName === '/'){
      if(queryData.id === undefined){
        fs.readdir('./home_button', 'utf8', function(error, filelist){
          ...
          var buttonList = templateListButton(filelist);
          var html = templateHTML(buttonList, title ,`<p>${description}</p>`);
          res.writeHead(200);
          res.end(html);
        });
      }
      else{
        fs.readFile(`home_button/${queryData.id}`,'utf8',function(error1, description){
            fs.readdir('./home_button', 'utf8', function(error2, filelist){
                var title = queryData.id;
                var buttonList = templateListButton(filelist);
                if(queryData.id === 'projects'){
                  fs.readdir(`home_button/projects`,function(error3, filelist){
                    var projectList = templateListProject(filelist);
                    var html = templateHTML(buttonList, title, projectList);
                    res.writeHead(200);
                    res.end(html);
                  });
                }
